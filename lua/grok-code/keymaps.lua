...
function M.register_keymaps(grok_code, config)
  ...
  if toggle.normal and toggle.normal ~= false then
    vim.keymap.set("n", toggle.normal, function()
      grok_code.toggle()
    end, { desc = "Toggle Grok Build", noremap = true, silent = true })
  end
  ...
  if km.send_range_ref and km.send_range_ref ~= false then
    ...
    vim.keymap.set("x", km.send_range_ref, function()
      ...
      desc = "Send @file:range (or single line) reference to grok terminal (visual)",
      noremap = true,
      silent = true,
    })
  end
